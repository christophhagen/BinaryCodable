...
final class PrimitiveEncodingTests: XCTestCase {
    ...
    func testFloatEncoding() throws {
        ...
        try compare(.greatestFiniteMagnitude, to: [0xFF, 0xFF, 0x7F, 0x7F])
        try compare(.zero, to: [0x00, 0x00, 0x00, 0x00])
        try compare(.pi, to: [0xDA, 0x0F, 0x49, 0x40])
        try compare(-.pi, to: [0xDA, 0x0F, 0x49, 0xC0])
        try compare(.leastNonzeroMagnitude, to: [0x01, 0x00, 0x00, 0x00])
    }
    ...
    func testDoubleEncoding() throws {
        ...
        print(-Double.pi)
        try compare(.greatestFiniteMagnitude, to: [0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xEF, 0x7F])
        try compare(.zero, to: [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])
        try compare(.pi, to: [0x18, 0x2D, 0x44, 0x54, 0xFB, 0x21, 0x09, 0x40])
        try compare(.leastNonzeroMagnitude, to: [ 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])
        try compare(-.pi, to: [0x18, 0x2D, 0x44, 0x54, 0xFB, 0x21, 0x09, 0xC0])
    }
}
